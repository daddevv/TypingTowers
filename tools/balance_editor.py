...
def simulate(cfg: dict, waves: int = 100) -> dict:
    ...
    mob_growth = cfg.get('mob_health_multiplier', 0)
    mob_armor = cfg.get('mob_base_armor', 0)
    mob_armor_mult = cfg.get('mob_armor_multiplier', 0)
    mob_speed = cfg.get('mob_base_speed', 1.0)
    mob_speed_mult = cfg.get('mob_speed_multiplier', 0)
    ...
    spawn_interval = cfg.get('mob_base_spawn_interval', 1.0)
    spawn_interval_mult = cfg.get('mob_spawn_interval_multiplier', 1.0)
    mobs_per_wave_base = cfg.get('mobs_per_wave_base', 3)
    mobs_per_wave_growth = cfg.get('mobs_per_wave_growth', 1)

    for w in range(1, waves + 1):
        hp = float(mob_base_hp) * (1 + (w - 1) * mob_growth)
        armor = float(mob_armor) * (1 + (w - 1) * mob_armor_mult)
        speed = float(mob_speed) * (1 + (w - 1) * mob_speed_mult)
        ...
        results['mob_armor'].append(armor)
        results['mob_speed'].append(speed)
...
def plot_results(results: dict, ax=None, canvas=None) -> None:
    ...
    if ax is None:
        ...
        ax = plt.gca()
    ax.clear()
    ax.plot(waves, results['mob_hp'], label='Mob HP')
    ax.plot(waves, results['mob_armor'], label='Mob Armor')
...
def run_gui():
    ...
    param_defs = [
        ('mob_base_health', 1, 1000, float),
        ('mob_health_multiplier', 1, 5, float),
        ('mob_base_armor', 0, 100, float),
        ('mob_armor_multiplier', 1, 5, float),
        ('mob_base_speed', 0.01, 1, float),
        ('mob_speed_multiplier', 1, 5, float),
        ...
        ('mob_base_spawn_interval', 0.1, 5, float),
        ('mob_spawn_interval_multiplier', 1, 5, float),
        ('mobs_per_wave_base', 1, 20, int),
        ('mobs_per_wave_growth', 0, 10, int),
    ]
    ...
    filter_options = {
        ...
        'TTK < Tsurvive': [
            'mob_base_health', 'mob_health_multiplier',
            'tower_base_damage', 'tower_damage_multiplier',
            'tower_base_fire_rate', 'tower_fire_rate_multiplier',
            'tower_base_reload_rate', 'tower_reload_rate_multiplier',
            'tower_base_range', 'tower_range_multiplier',
            'mob_base_speed', 'mob_speed_multiplier',
        ],
        'Alive Mobs <= Ammo': [
            'tower_base_ammo_capacity',
            'mob_base_spawn_interval', 'mob_spawn_interval_multiplier',
            'mobs_per_wave_base', 'mobs_per_wave_growth',
            'mob_base_health', 'mob_health_multiplier',
            'tower_base_damage', 'tower_damage_multiplier',
            'tower_base_fire_rate', 'tower_fire_rate_multiplier',
            'tower_base_reload_rate', 'tower_reload_rate_multiplier',
    ...
    for i, (key, vmin, vmax, typ) in enumerate(param_defs):
        ...
        var = tk.DoubleVar(value=cfg.get(key, vmin)) if typ is float else tk.IntVar(value=cfg.get(key, vmin))
        param_vars[key] = var
        scale = tk.Scale(root, variable=var, from_=vmin, to=vmax, resolution=0.01 if typ is float else 1, orient='horizontal', length=200)
    ...
    filter_menu = tk.OptionMenu(root, filter_var, *filter_options.keys(), command=update_filter)
    ...
    def update_plot(*args):
        ...
        warnings = []
        # TTK > Tsurvive check
        for w, ttk, tsurvive in zip(results['wave'], results['ttk'], results['tsurvive']):
            if ttk > tsurvive:
        ...
        for w, alive in zip(results['wave'], results['alive_mobs']):
            if alive > ammo_capacity:
                warnings.append(f"Wave {w}: Alive mobs ({alive}) > Ammo Capacity ({ammo_capacity})!")
                break
        ...
        selected = filter_var.get()
        plot_keys = {
            'All': ['mob_hp','mob_armor','ttk','tsurvive','alive_mobs','tower_damage','tower_fire_rate','tower_range','tower_splash','tower_crit'],
            'TTK < Tsurvive': ['mob_hp','ttk','tsurvive','tower_damage','tower_fire_rate','tower_range','mob_speed'],
            'Alive Mobs <= Ammo': ['alive_mobs','tower_base_ammo_capacity','mob_hp','ttk'],
        }
    ...
    warning_label = tk.Label(root, text='', fg='red', font=('Arial', 10, 'bold'), justify='left')
    warning_label.grid(row=len(param_defs), column=0, columnspan=4, sticky='w')

    # Reserve config fields for future upgrades/mechanics
    reserved_fields = [
        'future_upgrade_1', 'future_upgrade_2', 'future_mechanic_1', 'future_mechanic_2'
    ]
...
def main() -> None:
    ...
    parser.add_argument('--gui', action='store_true', help='Launch GUI for live editing')
